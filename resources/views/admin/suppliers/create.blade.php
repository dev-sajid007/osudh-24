...
@section('content')
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <textarea name="address" id="address" rows="3"
                                    class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 @error('address') border-red-300 @enderror">{{ old('address') }}</textarea>
                                @error('address')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <textarea name="notes" id="notes" rows="4" placeholder="Any additional notes about this supplier..."
                                    class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 @error('notes') border-red-300 @enderror">{{ old('notes') }}</textarea>
                                @error('notes')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
    ...
    @push('scripts')
        <script>
            // Auto-generate supplier code from company name
            document.getElementById('name').addEventListener('input', function() {
                const name = this.value;
                const codeField = document.getElementById('code');

                if (!codeField.value && name) {
                    const code = 'SUP' + name.toUpperCase()
                        .replace(/[^A-Z0-9]/g, '')
                        .substring(0, 3) + Math.random().toString(36).substring(2, 5).toUpperCase();
