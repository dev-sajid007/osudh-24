...
@section('content')
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="form" id="form"
                            ...
                            @foreach (['tablet', 'capsule', 'syrup', 'injection', 'cream', 'drops', 'powder'] as $form)
                                <option value="{{ $form }}"
                                    {{ old('form', $product->form) == $form ? 'selected' : '' }}>
                                    {{ ucfirst($form) }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="text" name="dosage" id="dosage" value="{{ old('dosage', $product->dosage) }}"
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" min="0" max="100" name="tax_rate"
                            id="tax_rate" value="{{ old('tax_rate', $product->tax_rate) }}"
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                    </div>
                </div>
                ...
                @if ($product->image_urls && count($product->image_urls) > 0)
                    <div id="current-images">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                            @foreach ($product->image_urls as $index => $imageUrls)
                                <div class="relative group">
                                    ...
                                    @if ($product->has_cloudinary_images)
                                        <div
                                            class="absolute top-1 right-1 bg-green-500 text-white text-xs px-1 py-0.5 rounded flex items-center space-x-1">
                                    ...
                                    @endif
                                    <!-- Future: Add delete button for individual images -->
                                </div>
                            @endforeach
                        </div>
                        <p class="text-sm text-gray-500 mt-2">
                            ...
                            @else
                                Local images - Consider reuploading to Cloudinary for better performance
                            @endif
                        </p>
                    </div>
                @endif
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">{{ old('description', $product->description) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="usage_instructions" id="usage_instructions" rows="2"
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">{{ old('usage_instructions', $product->usage_instructions) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="side_effects" id="side_effects" rows="2"
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">{{ old('side_effects', $product->side_effects) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="storage_instructions" id="storage_instructions" rows="2"
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">{{ old('storage_instructions', $product->storage_instructions) }}</textarea>
                    </div>
                </div>

                <!-- Add New Images and Tags -->
                <div class="space-y-6">
                    <div>
                        <label for="images" class="block text-sm font-medium text-gray-700 mb-4">Add New Images</label>
                        ...
                        <p class="mt-2 text-sm text-gray-500">New images will be uploaded to Cloudinary and added to
                            existing images</p>
                    </div>
                ...
                <div>
                    ...
                    <select name="status" id="status" required
                        ...
                        <option value="discontinued"
                            {{ old('status', $product->status) == 'discontinued' ? 'selected' : '' }}>Discontinued</option>
                    </select>
                </div>
            </div>
    ...
    @push('scripts')
        <script>
            // Enhanced multiple image preview functionality for product edit
            ...
            function updateDisplayState() {
                if (selectedFiles.length > 0) {
                    ...
                    // Dim current images to show they will be supplemented
                    if (currentImages) {
                        currentImages.style.opacity = '0.7';
                    }
                } else {
                    imagePreviewsContainer.classList.add('empty:hidden');
                    // Restore current images opacity
                    if (currentImages) {
                        currentImages.style.opacity = '1';
                    }
                }
            }
            ...
            // Dynamic Form Features (same as create form)
            ...
            function setupGenericSuggestions() {
                ...
                if (genericSelect) {
                    ...
                    genericSelect.addEventListener('change', function() {
                        ...
                        if (selectedOption.value && selectedOption.text !== 'Select Generic') {
                            // Update form suggestions based on generic change
                            console.log('Generic changed to:', selectedOption.text);
                        }
                    });
                }
            }
            ...
            if (nameField && formField) {
                nameField.addEventListener('input', generateSKU);
            ...
            if (unitPriceField) unitPriceField.addEventListener('input', calculateFinalPrice);
            if (discountField) discountField.addEventListener('input', calculateFinalPrice);
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // Calculate initial price if values exist
